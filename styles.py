...
.login-mini-stat strong{display:block;font-size:24px}
.login-mini-stat span{font-size:12px;color:#c7ddf2}

.login-form-header{padding:8px 4px 4px}
...
.login-logo-title{font-size:18px;font-weight:900;color:var(--text)}
.login-logo-sub{font-size:12px;color:var(--muted)}
.login-form-title{font-size:28px;font-weight:900;color:var(--text)}
.login-form-sub{font-size:14px;color:var(--muted);margin:5px 0 18px}
...
.brand{display:flex;align-items:center;gap:12px;padding:4px 2px 18px}
.brand-mark{
  width:44px;height:44px;border-radius:14px;
  background:linear-gradient(135deg,var(--cyan),var(--blue));
...
.role-title{
  ...
  color:#9bb5d0!important;margin:4px 2px 8px;
}

/* Visualizar como igual ao botão Sair */
...
[data-testid="stSidebar"] div[data-baseweb="select"] > div:hover{
  ...
  border-color:rgba(39,214,255,.45)!important;
}
div[role="listbox"]{
...
.menu-title-clean{
  ...
  color:#7fa0c2!important;margin:22px 6px 8px;
}

/* Menu compacto, sem bolinhas */
...
.hero-main h1{color:#fff;font-size:42px;line-height:1.05;margin:0 0 12px;letter-spacing:-.04em}
...
@media(max-width:900px){
  .login-visual-panel{border-radius:24px 24px 0 0;min-height:auto}
  [data-testid="stHorizontalBlock"] > [data-testid="stColumn"]:last-child
  div[data-testid="stVerticalBlockBorderWrapper"]{
    min-height:auto;border-left:1px solid var(--line)!important;
